...
import ValueTagModal from '../Modal/ValueTagModal'
import { authHeader } from '../../../helpers/authHeader'
import { handleResponse } from '../../../helpers/handleResponse'

class ValueTagDataTable extends Component {
   deleteItem = id => {
      let confirmDelete = window.confirm('Are you sure you want to delete this value tag?')
      if (confirmDelete) {
         const requestOptions = {
            method: 'DELETE',
            headers: { ...authHeader(), 'Content-Type': 'application/json' }
         };
         fetch('/valuetag/delete/' + id + ' ', requestOptions).then(handleResponse)
            .then(item => {
   ...
   activeInactiveTag = id => {
      ...
      fetch('/valuetag/active/inactive/' + id + ' ', requestOptions)
         .then(handleResponse)
         .then(item => {
            ...
            <div className="white-box">
               <div className="table-responsive">
                  <table className="table">
                     <thead>
                        <tr>
                           <th>Value Tag Icon</th>
                           <th>Value Tag Title</th>
                           <th>Value Tag Color</th>
                           <th>Date</th>
                              ...
                              <td>
                                 {
                                    tag.status === 1
                                       ? (<div className="label label-table label-success"
                                          onClick={() => this.activeInactiveTag(tag.id)}
...
export default ValueTagDataTable